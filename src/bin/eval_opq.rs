...
const OUTPUT_FILENAME: &str = "notebooks/data/eval_opq_results.csv";
...
fn run_benchmark(
    ...
    // 2. Train the optimized product quantizer.
    ...
    let seed = 42;
    let opq_iters = 5; // Additional iterations for the OPQ-specific step.
    let training_start = Instant::now();
    let opq =
        OptimizedProductQuantizer::fit(&training_data, m, k, max_iters, opq_iters, distance, seed);
    ...
    let reconstructed_data: Vec<Vector<f32>> = training_data
        ...
        .map(|vec| {
            ...
            let quantized = opq.quantize(vec);
